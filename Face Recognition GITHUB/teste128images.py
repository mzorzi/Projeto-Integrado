...
# Open video file
video_capture = cv2.VideoCapture("/home/victor/Downloads/short_hamilton_clip.mp4")
...
while video_capture.isOpened():
    ...
    if len(frames) == 32:
        batch_of_face_locations = face_recognition.batch_face_locations(frames, number_of_times_to_upsample=0, batch_size=32)

        # Now let's list all the faces we found in all 128 frames
        for frame_number_in_batch, face_locations in enumerate(batch_of_face_locations):
            number_of_faces_in_frame = len(face_locations)

            frame_number = frame_count - 32 + frame_number_in_batch
